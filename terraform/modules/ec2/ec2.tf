...
# }
data "aws_iam_instance_profile" "eks_access_role_instance_profile" {
  name = var.eks_access_role_instance_profile_name
}
...
resource "aws_instance" "ec2" {
  ...
  iam_instance_profile    = data.aws_iam_instance_profile.eks_access_role_instance_profile.name
...
# resource "null_resource" "upgrade_jenkins" {
#   provisioner "remote-exec" {
#     inline = ["sudo yum upgrade -y jenkins && touch upgrage.log && echo 'I updated Jankins' >> upgrade.log"]
#   connection {
#       type = "ssh"
#       user = "ec2-user"
#       private_key = file("/Users/elijahtorrence/.ssh/terransible")    
