...
resource "aws_instance" "bastion-host" {
  ...
  user_data = templatefile("../modules/bastion-host/user_data.tpl", {new_hostname = var.instance_name})
...
# Execute the shell script allowing for immediate ssh access
  provisioner "local-exec" {
    command               = "../modules/bastion-host/scripts/terraform_ssh_command.sh ${self.public_ip} ${var.private_key_path}"
  }

  # Execute the shell script to create and make executable open_jenkins files
  provisioner "local-exec" {
    command = "../modules/bastion-host/scripts/open_jenkins.sh ${self.public_ip}"
  }
